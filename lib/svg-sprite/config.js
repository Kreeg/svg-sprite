...
function SVGSpriterConfig(config) {
        ...
        if (stat) {
            ...
            for (const [m, value] of Object.entries(meta)) {
                if (_.isPlainObject(value)) {
                    this.shape.meta[path.join(path.dirname(m), path.basename(m, '.svg'))] = _.pick(value, ['title', 'description']);
                }
            }
        ...
        for (let transform of transforms) {
            ...
            if (_.isObject(transform)) {
                for (const [transformer, value] of Object.entries(transform)) {
                    if (value === true || _.isObject(value) || _.isFunction(value)) {
                        this.shape.transform.push([transformer, value === true ? {} : value]);
                        break;
                    }
                }
            }
    ...
    if ('transform' in this.svg) {
        if (_.isFunction(this.svg.transform)) {
            ...
            for (const transform of Object.values(this.svg.transform)) {
...
SVGSpriterConfig.prototype.filter = function(config = {}) {
    ...
    for (const [mode, value] of Object.entries(config)) {
        const configMode = _.isPlainObject(value) ? value : (value === true ? {} : null);
